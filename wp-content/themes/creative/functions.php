...
/** Theme Name	: Creative **/
	...
	require( get_template_directory() . '/functions/theme-page/admin-themes.php');
...
function creative_custom_admin_notice() {
	wp_register_style( 'custom_admin_css', get_template_directory_uri() . '/css/admin-rating.css');
	...
	<div class="notice-box notice-success is-dismissible flat_responsive_notice" data-dismissible="disable-done-notice-forever">
		<div>
			<p>	
			<?php _e('Thank you for using the free version of ','creative'); ?>
			<?php echo esc_html( $wl_th_info->get('Name') );?> - 
			<?php echo esc_html( $wl_th_info->get('Version') );
			 ?>
			<?php _e('Please give your reviews and ratings on ','creative'); echo $wl_th_info->get('Name'); _e(' theme. Your ratings will help us to improve our themes.', 'creative'); ?>
				<script type="text/javascript">alert(<?php echo $isitdismissed?>);</script>
			<a class="rateme" href="<?php echo esc_url('https://wordpress.org/support/theme/creative/reviews/?filter=5');  ?>" target="_blank" aria-label="Dismiss the welcome panel">
				<?php _e('Rate Us here','creative');?>
			</a>
...
add_action('admin_notices', 'creative_custom_admin_notice');
